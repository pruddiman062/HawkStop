...
{	
	...
	{
		var sIndex = 0;
		Ti.API.info(data_object[index].stops.length);
		for(j = 0; j<=data_object[index].stops.length-1 j++);
		{
			if(stopid == data_object[index].stops[j].Id)
			{
				return data_object[index].stops[j].schedule;
			}
		}
